# From train_all_nonpersonalized_new.py
# TO build a unified train script for MoE structure

# New : 
# nonpersonalized_new.py已经写好了
# 这版为了能支持多卡训练
...
def run_personalized_evaluation(args, model, tokenizer, logger):
...
def main(args, logger):
    ...
    # Eval Per User Start------------------------------------------------------------
    if args.rank == 0:
        run_personalized_evaluation(args, model, tokenizer, logger)
    # Eval Per User End------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    if rank == 0:
        time.sleep(1)  # 好像H100会不显示这个logger.info内容，所以加个sleep试试是不是异步阻塞问题
        logger.info("\n====== ARGUMENTS ======")
    ...
    try:
        ...
        print(f"⚠️ Rank {rank} Exception occurred:", repr(e))
