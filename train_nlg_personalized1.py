# From train_nlg_personalized.py
# New: 
# 1. 就是想看看为啥inference特别慢, 这个code是把max_new_tokens改成10了，快一点，其实不太对，
# 2. comment 掉了train 过程只看inference
# 在 train_all_nonpersonalized.py 里，其实发现了原因，誓要在tokenize里面把eos粘到response后面
...
def evaluate_on_test_set(model, tokenizer, test_file, device="cuda"):
    ...
    for example in tqdm(dataset, desc="Evaluating"):
        ...
        # 获取生成的答案
        pred_answer = pred.strip().split("####")[-1].strip().split()[0]
...
def main():
    ...
    path =  args.model
    tokenizer = transformers.LlamaTokenizer.from_pretrained(path, padding_side="right") #  padding_side="left"
    ...
    trainer = CustomTrainer(
        # scaling_factor=scaling_factor,   # to be removed
        model=model,
